function DynaSet()
{
    ...
    function separateExclusiveElements(inputContext)
    {
        ...
        for(var i=0; i< objectArray.length; i++)
        {
            if(!(objectArray[i] in allObjectsInfo))
            {
                ...
                allObjectsInfo_Reverse[inputContext.objects[objectArray[i]].name] = objectArray[i];
            }
    ...
    function computeInteractionEdges()
    {
        ...
        for(var index in window.interactionHyperEdges)
        {
            ...
            var tempEdge = {"nodes": hyperedgeSet, "nodesElementDictionaryArray":nodesElementDictionary, timestep: timestep, "authors": authorsArray, "title": e.title};
        ...
        for(var i =0; i<tempInteractionEdges.length; i++)
        {
            ...
            for(var j = 0; j<interactionEdges_Merged.length; j++)
            {
                ...
                if(setsAreEqual(e1.nodes, e2.nodes) && e1.timestep == e2.timestep)
                {
                    ...
                    interactionEdges_Merged[j]["nodesElementDictionaryArray"].push(e1["nodesElementDictionaryArray"]);
                    interactionEdges_Merged[j]["count"] = e2["count"]+1;
                }
            }
            if(!duplicateFound)
            {
                interactionEdges_Merged.push({"nodes":e1.nodes, timestep: e1.timestep, 
                                                "authors": new Array(e1["authors"]), 
                                                "nodesElementDictionaryArray": new Array(e1.nodesElementDictionaryArray),
        ...
        console.log("Number of interaction edges excluding self-loop: ", nonSelfLoopEdgeCount);
        console.log("Number of intra-group interaction edges: ", selfLoopEdgeCount);
        console.log("Total number of interactions: ", (nonSelfLoopEdgeCount+ selfLoopEdgeCount))
    ...
    function computeInteractionEdgesTech()
    {
        ...
        var timestepArray =  ["1990-1994", "1995-1999", "2000-2004", "2005-2009", "2010-2014", "2015-2019", "2020-2023"];
        for(var index in rawEdges)
        {
                            ...
                            if(concept.exclusive.length>0)
                            {
                                for(var t in concept.exclusive)
                                {
                                    ...
                                    if(exclusiveobj.objectid == elementId)
                                    {
                                        ...
                                        if(conceptid in nodesElementDictionary)
                                        {
                                            if(nodesElementDictionary[conceptid].indexOf(element)<0)
            ...
            var tempEdge = {"nodes": hyperedgeSet, "nodesElementDictionaryArray":nodesElementDictionary, "timestep": timestep, "authors": elementsArray, "title": e.details, "link": e.link};
        ...
        {
            if(edge.nodes.size >0)
                temp2InteractionEdges.push(edge);
    ...
    function computeGraphLayout ()
    {
        ...
        else if (window.sortBy == "numInteractions")
        {
            ...
            {
                conceptsInstabilityTuple.push([parseInt(conceptHash), conceptTupleDictionary[conceptHash]]);
            }
            ...
            while(remainingRows.length>0)
            {
                ...
                {
                    for(var k=0; k<remainingRows.length; k++)
                        similarityScoreArray.push([sortedRows[j], remainingRows[k], computeInteractionSimilarity(sortedRows[j], remainingRows[k])]);
                }
                similarityScoreArray.sort(function(a,b){
                    return b[2]-a[2];
                });
       ...
       for(var i=0; i< degreeDictionary["1"].length; i++)
       {
        ...
        // var widthOfOneZone = (width)/(numTimesteps+1);

        // if(window.currentDatasetId == "tech")
        //     widthOfOneZone = (width)/(numTimesteps+1);
        console.log(widthOfOneZone);
        ...
        for(var node in window.nodes)
        {
            ...
            if(temphash ==="added")
            {
                window.nodes[node].x = widthOfOneZone*(timestep+1) - widthOfOneZone*0.95;
            ...
            {
                window.nodes[node].x = widthOfOneZone*(timestep+1) + widthOfOneZone*0.4;
            ...
            else
            {
                ...
                if(!(temphash in verticalPosition))
                    console.log("Hash not found in verticalPosition", temphash, verticalPosition);
        ...
        function drawLeftColumn(svg, graphScrollGroup)
        {
            ...
            for(var deg in degreeDictionary)
            {
                ...
                window.shiftMainVisByX = guiParams.ksetGroupWidth - widthOfOneZone/2 + 20;
            ...
            for(var concept in verticalPosition)
            {
                ...
                var highlightYPos = verticalPositionRowHeight[pos][1] - verticalPositionRowHeight[pos][0] + maxHeightOfNode + pos*paddingBetweenNodes + tempDistanceFromTop + distanceFromAddedRemovedNodes - guiParams.additionalHeight/2;
                window.highlightRectPositions.push([ highlightYPos, verticalPositionRowHeight[pos][0] + guiParams.additionalHeight, "RowId"+concept ]);
                rowLegend.append("rect").attr({
                    ...
                    "y": highlightYPos,
                    "width": width,
                    "height": verticalPositionRowHeight[pos][0] + guiParams.additionalHeight,
                    "fill":"white",
                    "opacity":guiParams.highlightOpacity
                ...
                rowLegend.append("line").attr({
                    ...
                    "x2": guiParams.fontSize*(numSets+1) + (leftPadding+leftSpaceForDegreeGroup),
                    "y2": highlightYPos,
                    "stroke":"black",
                    "stroke-width":"0.5px"
                });
        ...
        for(var i=0; i<inputRawData.length; i++)
        {
            ...
            for(var deg in degreeDistributionPerTimeStep[i])
            {
                ...
                }).on("mouseenter", function()
                {
                    ...
                    var x = (parseInt(timestep))*widthOfOneZone + widthOfOneZone/2 - extendWidthEachSideBy;
                    var objectArray = window.degreeDistributionPerTimeStep[timestep][deg].objects;
                    var objectArrayCurated = [];
                    for(var ob of objectArray)
                    {
                        if("objectid" in ob)
                            objectArrayCurated.push(ob);
                    ...
                    var objectArray = window.degreeDistributionPerTimeStep[timestep][degree].objects;
                    var objectArrayCurated = [];
                    for(var ob of objectArray)
                    {
                        if("objectid" in ob)
        ...
        degreeDistributionRow.attr("transform","translate("+(window.shiftMainVisByX)+",0)");
        ...
        for(var i=0; i<inputRawData.length; i++)
        {
            ...
            var labelYpos = topPadding+heightOfDegreeDistributionBox+maxHeightOfNode;

            if(window.currentDatasetId == "tech")
                labelYpos += maxHeightOfNode;
            ...
            timestepLabel.append("rect").attr({
                ...
                "y": function(){
                        if(window.currentDatasetId != "image")
                            ...
                            return labelYpos - bbox.height + extra/2 ;
                    },
        ...
        timestepLabel.attr("transform","translate("+(window.shiftMainVisByX)+",0)");
        }

        // Start drawing
        var mainSvg = d3.select("#mainVisualization").append("svg").attr({
            "width": width + 200,
            "height": height * 4
        })
        ...
        for(var time in interactionEdges_MergedGrouped)
        {
            for(var e of interactionEdges_MergedGrouped[time])
            {
                ...
                e.nodes.forEach((conceptId) => 
                {
                    ...
                    var coords = [window.nodes[node].xBoundary, window.nodes[node].yBoundary];
        ...
        for(var time in interactionEdges_MergedGrouped)
        {
            ...
            for(var e of interactionEdges_MergedGrouped[time])
            {
                if(e.nodes.size>1)
                {
                    ...
                    var binIndex = 0;
                    for(var x=0; x<interactionEdges_MergedGroupedBinned[time].length; x++)
                    {
                        ...
                        {
                            if((e.minOrder >= e2.minOrder && e.minOrder < e2.maxOrder) ||
        ...
        var countDict = getMaxCountOfInteractionEdges(interactionEdges_MergedGrouped);
        ...
        {
            for(var binIndex =0; binIndex<interactionEdges_MergedGroupedBinned[time].length; binIndex++)
            {
                ...
                for(var ii=0; ii<edgesInBin.length; ii++)
                {
                    ...
                    if(e.nodes.size>1)
                    {
                        ...
                        var t = guiParams.nodeBarWidth + 2*guiParams.hyperEdgeCircleRadius +  guiParams.hyperEdgeCircleRadius*2.5*binIndex;
                        ...
                        e.nodes.forEach((conceptId) =>
                        classString = classString + " I"+conceptId);
                        e.class = classString;
                        e.circlePositions = [];
                        ...
                        e.circleClass = "interactionEdgesCircles "+e.edgeId +" C"+conceptId;
        ...
        nodeCreate.append("rect")
            ...
            .attr({
                ...
                "y": function (d) { return d.yBoundary; },
                // "width": widthOfNode,
                "width": guiParams.nodeBarWidth,
                "height": function (d) { return maxHeightOfNode;},
                "class": function (d) { return "nodeBarBoundary"; }
            })
            ...
            .attr({
                "id": function (d) { return d.nodeId; },
                "x": function (d) { return d.x+widthOfNode-guiParams.nodeBarWidth; },
                "y": function (d) { return d.yBoundary; },
                // "width": widthOfNode,
                "width": guiParams.nodeBarWidth,
                "height": function (d) { return maxHeightOfNode;},
        ...
        nodeCreate.append("rect")
            .attr({
                ...
                "rowId": function (d) { return d.setid; },
            ...
            .on("click", function (d) {
                ...
                elementNamesOnMouseOver(inputRawData[d.timestep][setid].exclusive, [d.x, d.y]);
        ...
        drawInteractionEdges(window.interactionEdges, window.interactionEdgesSelfLoop);
        ...
        var tempGraphWidth = width - guiParams.ksetGroupWidth - marginBeforeAndAfterText - 2*guiParams.fontSize;
        var renderedWidth = guiParams.widthOfOneZone * numTimesteps;
        window.xPosHorizontalScrollBar = guiParams.ksetGroupWidth +marginBeforeAndAfterText+ 2*guiParams.fontSize;
    ...
    function computeAndDrawGroupEdges(objectIds, group)
    {
        ...
        var selectedInteractionEdges = calculateSelectedInteractionEdges(objectIds, groupId);
        if(selectedInteractionEdges.normal.length>0 || selectedInteractionEdges.selfLoop.length > 0)
        {
            // console.log(selectedInteractionEdges);
            drawInteractionEdges(selectedInteractionEdges.normal, selectedInteractionEdges.selfLoop);
        }
    ...
    function elementNamesOnMouseOver(objectIdDictArray, pos)
    {
        d3.select("#elementNamesOnMouseOverGroup").remove();
        var objectIdArray = [];
        for(var ob of objectIdDictArray)
        ...
        for(var i=0; i<objectIdArray.length; i++)
        {
            if(textOverfillFlag == false)
            {
                ...
                if(i<breakCount)
                {
                    elementString = (i+1) + ". " +window.allObjectsInfo[objectIdArray[i]].name;
                }
                else
                {
                ...
                g.append("text").text(elementString).attr({
                    "x": pos[0] - 20,
    ...
    function highlightInteractionEdge(conceptId)
    {
        ...
        d3.selectAll(".I"+conceptId+":not(.selectedInteractionEdge)").attr("stroke","black");
        d3.selectAll(".I"+conceptId+":not(.selectedInteractionEdge)").attr("opacity",1);
    ...
    {
        d3.selectAll(".interactionEdges:not(.selectedInteractionEdge)").attr("opacity",guiParams.interactionEdgeOpacity);
        d3.selectAll(".interactionEdgesCircles").attr("opacity",guiParams.interactionEdgeOpacity);
        d3.selectAll(".interactionEdges:not(.selectedInteractionEdge)").attr("stroke","grey");
    ...
    function computeUniqueElements(edge)
    {
        ...
        for(var elementArray of edge["authors"])
        {
            ...
            {
                if(element in uniqueElementDict)
                    uniqueElementDict[element]++;
                else
                    uniqueElementDict[element] = 1;
            }
        }
    ...
    function drawInteractionEdges(edges, selfLoop)
    {
        ...
        var countDict = getMaxCountOfInteractionEdges(interactionEdges_MergedGrouped);
        window.interactionEdgeThicknessScale = d3.scale.linear()
                                                .domain([
                                                            1,
        ...
        var singleEdgeGroup = edgeGroup2.selectAll("g").data(edges).enter().append("g");
        singleEdgeGroup.attr({
            "class": function(e){return e.class;},
            "id": function(e){return e.edgeId;},
            "cursor":"pointer"
        }).on("click", function(d)
            {
                var text = "Elements who interacted across the exclusive intersections of ";
                var setNames="";
                var nodeArray = Array.from(d.nodes);
                for(var i=0; i<nodeArray.length; i++)
                {

                    if(i==1 && i== (nodeArray.length -1))
                        setNames += " and";
                    else if(i>1 && i== (nodeArray.length-1))
                        setNames += ", and";
                    else if(i>0 && i!= (nodeArray.length-1))
                        setNames += ",";

                    var tempIntersectionNameString = window.sethashandNameDict[nodeArray[i]];
                    if(tempIntersectionNameString.charAt(tempIntersectionNameString.length -1) == ",")
                        tempIntersectionNameString = tempIntersectionNameString.substring(0,tempIntersectionNameString.length -1);
                ...
                {
                    objectsArray.push({"objectid": window.allObjectsInfo_Reverse[obj], "weight": d.uniqueElements[obj]});
                }
                if($(`#radioSelection`).prop("checked"))
                {
            ...
            .append("title").text(function(e){
                return e.count+" inter-group interactions; " + e.numUniqueElements + " unique elements";
            });
        function interactionEdgeMouseEnter(d, edgeType)
        {
            d3.select("#interactionEdgeMouseOver").remove();
                ...
                if(edgeType == "normal")
                {
                    ...
                    {
                        disArray.push(Math.abs(d.circlePositions[i+1][1] - d.circlePositions[i][1]));
                    }
                ...
                for(var x=0; x<d.nodesElementDictionaryArray.length; x++)
                {
                    if(textOverfillFlag == false)
                    {
                        ...
                        for(var conceptId in edge)
                        {
                            ...
                            {
                                elementString = "...[" + (d.nodesElementDictionaryArray.length - x) +" more]";
                                textOverfillFlag = true;
                                // break;
                            }
        ...
        singleEdgeGroup.append("line").attr({
                        "x1": function(e){ return e.x1 ;},
                        "y1": function(e){ return e.y1 ;},
                        "x2": function(e){ return e.x2 ;},
                        "y2": function(e){ return e.y2 ;},
                        "edgeId": function(e){ return e.edgeId ;},
                        "stroke-opacity": function(e){ 
                            if(e.class.includes("group0"))
                            {
                                ...
                                return guiParams.interactionEdgeOpacity; 
                            },
                        "stroke-width": function(e){ 
                            if(e.class.includes("group0"))
                            {
                                ...
                                return interactionEdgeThicknessScale(e.count); 
                            }
                    });
        singleEdgeGroup.append("line").attr({
            "x1": function(e){ return e.x1 ;},
        ...
        singleEdgeGroup.selectAll("circle").data(function(e){
        ...
        var group = selfLoopEdgeGroup.selectAll("g").data(selfLoop).enter().append("g")
        .attr("class", function(e){ return e.class; })
        .attr("id", function(e){ return e.edgeId; })
        .on("mouseenter", function(d){
            // var coordinates= d3.mouse(this);
            // var x = coordinates[0];
            // var y = coordinates[1];
            ...
            d3.select("#interactionEdgeMouseOver").remove();
                ...
                group.append("rect").attr({
                    ...
                    "width": function(e){ 
                        if(e.class.includes("group0"))
                            ...
                            return interactionEdgeThicknessScale(e.count);
                        ;},
                    "height": function(e){ return e.height ;},
                    "transform": function(e){ return `translate(${e.x} ${e.y}) skewX(-45) translate(${-e.x} ${-e.y})`;},
                ...
                {
                    var text = "Elements who interacted within the exclusive intersection of ";
                    var setNames="";
                    var nodeArray = Array.from(d.nodes);
                    for(var i=0; i<nodeArray.length; i++)
                    {
                    ...
                    return e.count+" intra-group interactions; "+ e.numUniqueElements+" unique elements";
                });
    ...
    function drawDetails(type, selection, timestep, timestepArray, nodeid)
    {
        ...
        if(type == "interaction")
        {
            ...
            selection.nodes.forEach((conceptId) => {
                ...
                if(tmparray.length ==1)
                    ...
                    selectionString += " interactions between exclusive intersections "
        ...
        else if(type == "node")
        {
            ...
            for(var ob of selection)
            {
                ...
                if(window.currentDatasetId == "tech")
                {
                    var timestepInfo = window.techCompaniesRawJSON.memberships[window.filenames[timestep]];
                    if(obName in timestepInfo)
                    {
                        var details = timestepInfo[obName].details;
                        if(details != "")
                            li.append("p").append("text").text(details);
                    }
                }
            }
        ...
        else if(type == "edge")
        {
            ...
            for(var ob of selection)
            {
                ...
                if(window.currentDatasetId == "tech")
                {
                    var detail1 = window.techCompaniesRawJSON.memberships[window.filenames[timestepArray[0]]][obName].details;
                    var detail2 = window.techCompaniesRawJSON.memberships[window.filenames[timestepArray[1]]][obName].details;
                    if(timestepArray[0] == timestepArray[1])
                    {
                        // li.append("text").text(": "+detail1);
                        li.append("p").append("text").text(window.filenames[timestepArray[0]] + ": "+detail1);
                    }
                    else
                    {
                        if(detail1 != "")
                            li.append("p").append("text").text(window.filenames[timestepArray[0]] + ": "+detail1);
                        if(detail2 != "")
                            li.append("p").append("text").text(window.filenames[timestepArray[1]] + ": "+detail2);

                    }
                }
                
            }   
            selectionString += " edge between timesteps " + window.filenames[timestepArray[0]] + " and " + window.filenames[timestepArray[1]]; 
        ...
        else if(type == "element")
        {
            ...
            if(window.currentDatasetId == "tech")
            {
                ...
                {
                    var timestepInfo = window.techCompaniesRawJSON.memberships[timestepString];
                    if(obName in timestepInfo)
                    {
                        var details = timestepInfo[obName].details;
                        if(details != "")
    ...
    function getMaxCountOfInteractionEdges(interactionEdges_MergedGrouped)
    {
        ...
        return {"minCount": minCount, "maxCount": maxCount, "minCountLoop": minCountLoop, "maxCountLoop": maxCountLoop};
    }
    ...
    function calculateSelectedInteractionEdges(objectIdsArray, groupId)
    {
        ...
        {
           tempInteractionEdges = commonComputationOfInteractionEdges(objectIdsArray, window.interactionEdges, groupId);
           tempInteractionEdgesSelfLoop = commonComputationOfInteractionEdges(objectIdsArray, window.interactionEdgesSelfLoop, groupId);
        }
        return {"normal": tempInteractionEdges, "selfLoop":tempInteractionEdgesSelfLoop};
    }

    function commonComputationOfInteractionEdges(objectIdsArray, iEdges, groupId)
    {
        var tempInteractionEdges = [];
        ...
        for(var e of iEdges)
        {
            ...
            for(var objectArray of e.authors)
            {
                ...
                const intersection = new Set([...tempSetofObjectsInInteraction].filter((x) => setOfObjectNames.has(x)));
